		...
		struct sunxi_ahub_daudio_priv *sunxi_ahub_daudio, int id)
{
...
#if defined(CONFIG_SND_SOC_AC108)
	/* for loopback */
	if (sunxi_ahub_daudio->tdm_num == SUNXI_AHUB_I2S0_ID) {
		regmap_write(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_I2S_OUT_CH0MAP0(id), 0x01234576);
	} else
#endif
	{
		regmap_write(sunxi_ahub_daudio->regmap,
	...
	if (sunxi_ahub_daudio->tdm_num == SUNXI_AHUB_I2S0_ID) {
		regmap_write(sunxi_ahub_daudio->regmap,
			SUNXI_AHUB_I2S_IN_CHMAP0(id), 0x76542301);
	} else
#endif
	{
		regmap_write(sunxi_ahub_daudio->regmap,
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
...
#if defined(CONFIG_SND_SOC_AC108)
	/* we need set the config by sysconfig */
	if (sunxi_ahub_daudio->tdm_num == SUNXI_AHUB_I2S0_ID) {
		sunxi_ahub_daudio_init_fmt(sunxi_ahub_daudio,
			(sunxi_ahub_daudio->audio_format
				| (sunxi_ahub_daudio->signal_inversion << 8)
...
#if defined(CONFIG_SND_SOC_AC108)
	regmap_update_bits(sunxi_ahub_daudio->regmap,
				SUNXI_AHUB_I2S_CHCFG(0),
				(0x1 << I2S_CHCFG_TX_STATE),
				(0x1 << I2S_CHCFG_TX_STATE));
#endif
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sunxi_ahub_daudio->tdm_num == SUNXI_AHUB_HDMI_ID) {
			...
			if (sunxi_ahub_daudio->tdm_num == SUNXI_AHUB_I2S0_ID) {
				regmap_update_bits(sunxi_ahub_daudio->regmap,
					SUNXI_AHUB_I2S_OUT_SLOT0(sunxi_ahub_daudio->tdm_num),
					(0x0f << I2S_OUT_SLOT_NUM),
					(0x7 << I2S_OUT_SLOT_NUM));
				regmap_update_bits(sunxi_ahub_daudio->regmap,
					SUNXI_AHUB_I2S_OUT_SLOT0(sunxi_ahub_daudio->tdm_num),
					(0xffff<<I2S_OUT_SLOT_EN),
					(0x3 << 0x6));
...
#if defined(CONFIG_SND_SOC_AC108)
		/* it set the 8 channel slot for input */
		if ((sunxi_ahub_daudio->ac108_used)
			&& (sunxi_ahub_daudio->tdm_num == SUNXI_AHUB_I2S0_ID)) {
			regmap_update_bits(sunxi_ahub_daudio->regmap,
				SUNXI_AHUB_I2S_CHCFG(sunxi_ahub_daudio->tdm_num),
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		/* HDMI module, we just keep this clk */
		break;
	default:
		return -EINVAL;
	}
...
#if defined(CONFIG_SND_SOC_AC108)
	ret = of_property_read_u32(np, "ac108_used", &temp_val);
	if (ret < 0)
		sunxi_ahub_daudio->ac108_used = 0;
	else
		sunxi_ahub_daudio->ac108_used = temp_val;
#endif
