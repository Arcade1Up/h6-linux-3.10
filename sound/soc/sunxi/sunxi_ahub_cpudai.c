		...
		struct sunxi_ahub_cpudai_priv *sunxi_ahub_cpudai)
{
	...
	for (i = 0; i < 4; i++) {
		reg_val = sunxi_ahub_read(SUNXI_AHUB_I2S_RXCONT(i));
		if (reg_val & (1<<19)) {
			/* I2S0...HDMI...I2S2...CVBS... */
			reg_bit = 23 - i;
		...
		struct sunxi_ahub_cpudai_priv *sunxi_ahub_cpudai)
{
	...
	if (sunxi_ahub_cpudai->karaoke_mode) {
		...
		case 0:
			...
			/* disable for loopback HDMI data */
			sunxi_ahub_update_bits(SUNXI_AHUB_I2S_CTL(0),
					(1<<I2S_CTL_TXEN), (0<<I2S_CTL_TXEN));
			break;
		case 1:
				...
				struct snd_soc_dai *dai)
{
	...
	/* fix the "PA" audio when play music first time */
	/* modify ahub rst and clock from trigger to startup */
	switch (sunxi_ahub_cpudai->id) {
	case 0:
		/* operation HDMI I2S module */
		sunxi_ahub_update_bits(SUNXI_AHUB_RST,
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			sunxi_ahub_i2s_capture_route_enable(sunxi_ahub_cpudai);
			sunxi_ahub_update_bits(
				SUNXI_AHUB_APBIF_RX_CTL(sunxi_ahub_cpudai->id),
				(1 << APBIF_RX_START),
				(1 << APBIF_RX_START));
			sunxi_ahub_update_bits(
				SUNXI_AHUB_APBIF_RX_IRQ_CTL(sunxi_ahub_cpudai->id),
				(1 << APBIF_RX_DRQ),
				(1 << APBIF_RX_DRQ));
		}
		break;
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			sunxi_ahub_i2s_playback_route_disable(
				sunxi_ahub_cpudai);
			sunxi_ahub_update_bits(
				SUNXI_AHUB_APBIF_TX_IRQ_CTL(sunxi_ahub_cpudai->id),
			...
			sunxi_ahub_i2s_capture_route_disable(sunxi_ahub_cpudai);
			sunxi_ahub_update_bits(
				SUNXI_AHUB_APBIF_RX_IRQ_CTL(sunxi_ahub_cpudai->id),
