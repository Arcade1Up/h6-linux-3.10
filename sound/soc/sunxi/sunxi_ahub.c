		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	struct sunxi_hdmi_priv *sunxi_hdmi =
				snd_soc_card_get_drvdata(dai->card);
...
static int sunxi_ahub_apbif_txdif_set_gat_rst(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai, int id, bool enable)
{
	struct sunxi_ahub_priv *sunxi_ahub =
				snd_soc_codec_get_drvdata(dai->codec);
	unsigned int APBIF_TXDIF_RST = 0;
	unsigned int APBIF_TXDIF_GAT = 0;

	switch (id) {
	case 0:
		APBIF_TXDIF_RST = APBIF_TXDIF0_RST;
		APBIF_TXDIF_GAT = APBIF_TXDIF0_GAT;
		break;
	case 1:
		APBIF_TXDIF_RST = APBIF_TXDIF1_RST;
		APBIF_TXDIF_GAT = APBIF_TXDIF1_GAT;
		break;
	case 2:
		APBIF_TXDIF_RST = APBIF_TXDIF2_RST;
		APBIF_TXDIF_GAT = APBIF_TXDIF2_GAT;
		break;
	default:
		pr_warn("[%s] not support the id number.\n", __func__);
		return -EINVAL;
	}
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (dai->id == 0) {
				sunxi_ahub_apbif_txdif_set_gat_rst(substream,
					dai, dai->id, true);
			}
		} else if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE) &&
			(dai->id == 0)) {
			regmap_read(sunxi_ahub->regmap, SUNXI_AHUB_RST, &reg_val);
			if (!((reg_val >> APBIF_RXDIF0_RST) & 0x1))
				regmap_update_bits(sunxi_ahub->regmap, SUNXI_AHUB_RST,
						(0x1 << APBIF_RXDIF0_RST),
						(0x1 << APBIF_RXDIF0_RST));
			regmap_read(sunxi_ahub->regmap, SUNXI_AHUB_GAT, &reg_val);
			if (!((reg_val >> APBIF_RXDIF0_GAT) & 0x1))
				regmap_update_bits(sunxi_ahub->regmap, SUNXI_AHUB_GAT,
						(0x1 << APBIF_RXDIF0_GAT),
						(0x1 << APBIF_RXDIF0_GAT));
			regmap_write(sunxi_ahub->regmap,
				SUNXI_AHUB_I2S_IN_CHMAP0(dai->id), 0x67542301);
		}
		break;
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			(dai->id == 0)) {
			regmap_write(sunxi_ahub->regmap,
				SUNXI_AHUB_I2S_IN_CHMAP0(dai->id), 0x76543210);
			regmap_update_bits(sunxi_ahub->regmap, SUNXI_AHUB_GAT,
					(0x1 << APBIF_RXDIF0_GAT),
					(0x0 << APBIF_RXDIF0_GAT));
			regmap_update_bits(sunxi_ahub->regmap, SUNXI_AHUB_RST,
					(0x1 << APBIF_RXDIF0_RST),
					(0x0 << APBIF_RXDIF0_RST));
		}
		break;
	default:
		return -EINVAL;
...
static struct snd_soc_codec_driver soc_ahub_dev_sunxi = {
	...
	.controls = ahub_controls,
	.num_controls = ARRAY_SIZE(ahub_controls),
