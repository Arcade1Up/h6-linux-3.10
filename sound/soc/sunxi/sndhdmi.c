	...
	struct snd_soc_dai *dai)
{
	...
	struct sunxi_hdmi_priv *sndhdmi_priv = snd_soc_card_get_drvdata(card);
	struct sndhdmi_priv *sunxi_hdmi = snd_soc_codec_get_drvdata(dai->codec);
	...
	if (sunxi_hdmi->hdmi_para.sample_rate != params_rate(params)) {
		sunxi_hdmi->hdmi_para.sample_rate = params_rate(params);
		sunxi_hdmi->update_param = 1;
	}
	if (sunxi_hdmi->hdmi_para.channel_num != params_channels(params)) {
		sunxi_hdmi->hdmi_para.channel_num = params_channels(params);
		sunxi_hdmi->update_param = 1;
	}
	if (sunxi_hdmi->hdmi_para.data_raw != sndhdmi_priv->hdmi_format) {
		sunxi_hdmi->hdmi_para.data_raw = sndhdmi_priv->hdmi_format;
		sunxi_hdmi->update_param = 1;
	}
	...
	*/
	...
	if (sunxi_hdmi->hdmi_para.sample_bit != hdmi_para.sample_bit) {
	...
	if (sunxi_hdmi->hdmi_para.ca != hdmi_para.ca) {
...
//#endif
	...
	if (sunxi_hdmi->update_param ||
		/*
		((hdmi_para.data_raw > 1) || (hdmi_para.sample_bit!=16) ||
		...
		*/
		...
		pr_warn("hdmi pcm_count_num:%d, update_param:%d\n",
			atomic_read(&pcm_count_num), sunxi_hdmi->update_param);
	}
	...
	*/
	...
	if (hdmiaudio_reset_en) {
		pr_err("%s,l:%d,hdmi_para.data_raw:%d, "
			"hdmi_para.sample_bit:%d, hdmi_para.channel_num:%d, "
			"hdmi_para.sample_rate:%d, hdmiaudio_reset_en:%d\n",
			__func__, __LINE__, hdmi_para.data_raw,
			hdmi_para.sample_bit, hdmi_para.channel_num,
			hdmi_para.sample_rate, hdmiaudio_reset_en);
	}
