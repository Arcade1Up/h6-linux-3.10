...
build_modules()
{
	...
	NAND_DRIVER=`cat .config | grep "CONFIG_SUNXI_NAND" || echo ""`
	...
	if [[ "$NAND_DRIVER" == "CONFIG_SUNXI_NAND=y" ]]; then
		echo "[NAND] CONFIG_SUNXI_NAND=y"
		echo "[NAND] Nand Driver Already Built in Kernel..."
	else
		echo "[NAND] CONFIG_SUNXI_NAND=m or is not set"
		echo "[NAND] Nand Driver Will Built As Modules..."
		build_nand_lib
