...
#ifdef SPREAD_SPECTRUM_CLOCKING_FEATURE
struct hdmi_clk_config hdmi_clk_cfg[] = {
		{27000, {0x03001048, 0x81006200}, {0x03001148, 0xc3a00f5c}, {0x03001348, 0},
			    {0x03001b80, 0x8300030a}, {0x03001b00, 0x8200030a}, {0x03001b1c, 0x00030001},},
		{74250, {0x03001048, 0x81006200}, {0x03001148, 0xc3a00f5c}, {0x03001348, 0},
				{0x03001b80, 0x83000207}, {0x03001b00, 0x82000207}, {0x03001b1c, 0x00030001},},
		{148500, {0x03001048, 0x81006200}, {0x03001148, 0xc3a00f5c}, {0x03001348, 0},
				{0x03001b80, 0x8300000f}, {0x03001b00, 0x8200000f}, {0x03001b1c, 0x00030001},},
		{297000, {0x03001048, 0x81003000}, {0x03001148, 0xc070fae1}, {0x03001348, 0},
				{0x03001b80, 0x83000003}, {0x03001b00, 0x82000001}, {0x03001b1c, 0x00030001},},
		{594000, {0x03001048, 0x81003000}, {0x03001148, 0xc070fae1}, {0x03001348, 0},
				{0x03001b80, 0x83000003}, {0x03001b00, 0x82000001}, {0x03001b1c, 0x00030001},},
};

static void physical_writel(unsigned int phy_addr,
				unsigned int val)
{
		unsigned int __iomem *addr;

		addr = ioremap(phy_addr, 16);
		writel(val, (void __iomem *)(addr));
		iounmap(addr);
}

void hdmi_clk_enable_correct(void)
{
	unsigned int clk_rate, i;

	if (!hdmi_drv->hdmi_core)
		return;
	clk_rate = hdmi_drv->hdmi_core->mode.pVideo.mDtd.mPixelClock;
	if (!clk_rate)
		return;

	for (i = 0; i < (sizeof(hdmi_clk_cfg) / sizeof(struct hdmi_clk_config)); i++) {
		if (hdmi_clk_cfg[i].pixel_clk == clk_rate) {
			physical_writel(hdmi_clk_cfg[i].parent_clk.addr,
				hdmi_clk_cfg[i].parent_clk.val);
			physical_writel(hdmi_clk_cfg[i].parent_pctl.addr,
				hdmi_clk_cfg[i].parent_pctl.val);
			physical_writel(hdmi_clk_cfg[i].parent_bias.addr,
				hdmi_clk_cfg[i].parent_bias.val);
			physical_writel(hdmi_clk_cfg[i].tcon_clk.addr,
				hdmi_clk_cfg[i].tcon_clk.val);
			physical_writel(hdmi_clk_cfg[i].hdmi_clk_reset.addr,
				hdmi_clk_cfg[i].hdmi_clk_reset.val);
			physical_writel(hdmi_clk_cfg[i].hdmi_clk.addr,
				hdmi_clk_cfg[i].hdmi_clk.val);
		}
	}
}

void hdmi_clk_disable_correct(void)
{
	unsigned int clk_rate, i;
	...
	for (i = 0; i < (sizeof(hdmi_clk_cfg) / sizeof(struct hdmi_clk_config)); i++) {
		if (hdmi_clk_cfg[i].pixel_clk == clk_rate) {
			physical_writel(hdmi_clk_cfg[i].hdmi_clk_reset.addr, 0);
		}
	}
}
#endif
...
static s32 hdmi_enable(void)
{
	...
	if (hdmi_enable_mask == 1) {
		...
		pr_err("[error]hdmi should be disabled before enabled");
		return 0;
	}

    /* add for Skyworth HDMI Test */
#ifdef SPREAD_SPECTRUM_CLOCKING_FEATURE
	hdmi_clk_enable_correct();
#endif
...
static void hdmi_plugin_proc(void)
{
	...
	mutex_unlock(&hdmi_drv->ctrl_mutex);
#if defined(CONFIG_SWITCH) || defined(CONFIG_ANDROID_SWITCH)
	if ((!(hdmi_hpd_mask & 0x110)) && hdmi_drv->hdmi_core->mode.edid_done)
		switch_set_state(&hdmi_switch_dev, 1);
#endif
	edid_correct_hardware_config();
...

{
	...
	if (of_property_read_u32(pdev->dev.of_node,
				       ...
				       &hdmi_drv->is_cts))
		pr_info("ERROR: can not get hdmi_cts_compatibility\n");
				...
				char *buf)
{
	...
	mutex_lock(&hdmi_drv->hdcp_mutex);
	HDCP_INF("hdcp_encrypt_status:%u\n", hdcp_encrypt_status);
