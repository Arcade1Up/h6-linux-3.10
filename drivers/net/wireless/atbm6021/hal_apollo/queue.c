				     ...
				     struct atbm_queue_item *item)
{
	...
	gc_item = kmalloc(sizeof(struct atbm_queue_item),
			GFP_ATOMIC);
	BUG_ON(!gc_item);
...
{
	struct atbm_queue_stats *stats = queue->stats;
	struct atbm_queue_item *item = NULL,*tmp_item = NULL;
	struct atbm_vif *priv;
	int if_id;
	bool wakeup_stats = false;
	struct atbm_txpriv *txpriv;
	...
	if (queue->overfull[if_id_clear]) {
		...
		if (queue->num_queued_vif[if_id_clear] <= capacity) {
			...
			unsigned long tmo = item->queue_timestamp + queue->ttl;
			WARN_ON(item->txpriv.if_id != if_id_clear);
		      ...
		      unsigned long ttl)
{
	...
	#endif
	queue->pool = kzalloc(sizeof(struct atbm_queue_item) * capacity,
			GFP_KERNEL);
	if (!queue->pool)
		return -ENOMEM;
