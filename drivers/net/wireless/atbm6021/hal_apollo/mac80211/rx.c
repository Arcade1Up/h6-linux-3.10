		     ...
		     struct ieee80211_rate *rate)
{
	...
	if(local->only_monitors&&local->monitor_sdata){
		...
		monitor_skb = skb_clone(skb, GFP_ATOMIC);
					     ...
					     struct sk_buff *skb)
{
	...
	if(skb->len < 100){
		Sta_shortTcp_RxCnt++;
		if(Sta_shortTcp_RxCnt > 100){
			HT_RX_REORDER_BUF_TIMEOUT = HZ/10;
		}
	}
	else {
					   ...
					   size_t len)
{
	...
	resp = (struct atbm_ieee80211_mgmt *) skb_put(skb, 24);
	memset(resp, 0, 24);
...
ieee80211_rx_h_userspace_mgmt(struct ieee80211_rx_data *rx)
{
	...
	if(ieee80211_is_assoc_req(((struct atbm_ieee80211_mgmt *)rx->skb->data)->frame_control)){
		...
		u8* rxmgm_buf = kmalloc(rxmgm_len+sizeof(atbm_event), GFP_ATOMIC);
