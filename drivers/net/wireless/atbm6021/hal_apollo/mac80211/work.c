		...
		if(scan_timeout || list_empty(&local->work_list)){
			if(scan_timeout){
				if (local->scan_req != local->int_scan_req){
					...
					cfg80211_scan_done(local->scan_req, false);
					local->scanning = 0;
					local->scan_req = NULL;
...
static void ieee80211_work_work(struct work_struct *work)
{
	...
	list_for_each_entry_safe(wk, tmp, &local->work_list, list) {
		...
		if (!started && !chan_state->tmp_channel) {
			 ...
			 */
			printk(KERN_ERR "%s:start work ch(%d)\n",__func__,wk->chan->hw_value);
...
//				ieee80211_ready_on_channel(&local->hw);
	...
	if(local->scan_req &&!local->scanning){
		...
		scan_timeout = !time_is_after_jiffies(local->pending_scan_start_time+(2*HZ));
