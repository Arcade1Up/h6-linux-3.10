...
static void sta_set_sinfo(struct sta_info *sta, struct station_info *sinfo)
{
	...
	if ((sta->local->hw.flags & IEEE80211_HW_SIGNAL_DBM) ||
		...
		#else
		sinfo->filled |= NL80211_STA_INFO_SIGNAL | NL80211_STA_INFO_SIGNAL_AVG;
		#endif
				   ...
				   struct beacon_parameters *params)
{
	...
	new = kzalloc(sizeof(struct associate_data)+params->assocresp_ies_len, GFP_KERNEL);
	if(!new)
		return -ENOMEM;
							...
							struct cfg80211_ap_settings *settings)
{
	...
	ieee80211_set_channel(wiphy,dev,chan,chtype);
	#endif
	...
	)
{
	...
	if(ETF_Test_is_Start()||LOCAL_IS_CONNECTTING(sdata->local)){
			   ...
			   u16 status_code, struct sk_buff *skb)
{
	...
	#endif
		#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 1, 0))
		skb_put(skb, 1 + sizeof(struct tdls_discover_resp_s));
		#else
