...
int atbm_bh_schedule_tx(struct atbm_common *hw_priv){
	...
	#else
	if((hw_priv->txrx_workqueue==NULL)||(atbm_bh_is_term(hw_priv)))
	{
		return -1;
	}
	queue_work(hw_priv->txrx_workqueue,&hw_priv->tx_complete_work);
	#endif
	return 0;
}
...
#define RX_ALLOC_BUFF_OFFLOAD (  (36+16)/*RX_DESC_OVERHEAD*/+4/*FCS_LEN*/ -16 /*WSM_HI_RX_IND*/)
	while ((skb = skb_dequeue(&hw_priv->rx_frame_queue)) != NULL) {
		...
		skb_queue_tail(&hw_priv->rx_frame_free, skb);
		
	}
	while ((skb = skb_dequeue(&hw_priv->rx_frame_free)) != NULL) {
		/*atbm transmit packet to device*/			
		hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
...
int atbm_bh_schedule_rx(struct atbm_common *hw_priv){
	...
	queue_work(hw_priv->txrx_workqueue,&hw_priv->rx_complete_work);
	#endif
	return 0;
}
#ifndef USB_USE_TASTLET_TXRX
...
int atbm_device_txrx_test_init(struct atbm_common *hw_priv,int caseNum)
{
	if(caseNum ==1){
		...
		test_skb_rx_id = 0;
		test_skb[0] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[1] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[2] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[3] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[4] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[5] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[6] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		test_skb[7] = dev_alloc_skb(MAX_TX_TEST_LEN+64);
		atbm_device_rx_test(hw_priv);
		atbm_device_rx_test(hw_priv);
		atbm_device_tx_test(hw_priv);
		atbm_device_tx_test(hw_priv);
