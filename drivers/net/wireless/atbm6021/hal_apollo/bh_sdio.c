...
void atbm_reset_sta_send_deauthen(struct atbm_common *hw_priv,u8 mask)
{
	...
	while ((deauth_skb = __skb_dequeue(&frames)))
	{
...
rx_continue:
	...
	if(atbm_bh_is_term(hw_priv) || atomic_read(&hw_priv->bh_term)){
		...
		skb_queue_tail(&hw_priv->rx_frame_queue, skb_rx);
	}
	read_len_lsb = (ctrl_reg & ATBM_HIFREG_CONT_NEXT_LEN_LSB_MASK)*2;
	read_len_msb = (ctrl_reg & ATBM_HIFREG_CONT_NEXT_LEN_MSB_MASK)*2;
	read_len=((read_len_msb>>2)+read_len_lsb);
...
int atbm_rx_bh_flush(struct atbm_common *hw_priv)
{
	...
	while ((skb = skb_dequeue(&hw_priv->rx_frame_queue)) != NULL) {
...
int atbm_device_wakeup(struct atbm_common *hw_priv)
{
	...
	while(1){
		...
		if (ctrl_reg & ATBM_HIFREG_CONT_RDY_BIT) {
			bh_printk(KERN_DEBUG "[BH] Device awake.<%d>\n",loop);
			ret= 1;
			goto __wup_exit;
		}
		//wait 1S,else fail
		if(loop++ > 4000){
			ret = atbm_bh_read_ctrl_reg(hw_priv, &ctrl_reg);
			bh_printk(KERN_DEBUG "[BH] Device wakeup Fail<%d> %x\n",loop,ctrl_reg);
			ret= -1;
			goto __wup_exit;
		}
	}
...
#define RX_ALLOC_BUFF_OFFLOAD (  (40+16)/*RX_DESC_OVERHEAD*/ -16 /*WSM_HI_RX_IND*/)
		...
		if(wsm_id == WSM_MULTI_RECEIVE_INDICATION_ID){
			...
			do {
				...
				skb_copy = dev_alloc_skb(wsm_len + 16);
				
				skb_reserve(skb_copy,  (8 - (((unsigned long)skb_copy->data)&7))/*ALIGN 8*/);
				
				memmove(skb_copy->data, wsm, wsm_len);
				skb_put(skb_copy,wsm_len);
...
static int atbm_bh(void *arg)
{
	...
	AggrBuffer= kmalloc(SDIO_TX_MAXLEN,GFP_KERNEL | GFP_DMA);
	if (!AggrBuffer) {
		...
		#define WSM_SDIO_TX_MULT_BLOCK_SIZE	(SDIO_BLOCK_SIZE)
			...
			do {
				...
				if (ret <= 0) {
					...
					//	printk("wsm_tx->id %d seq %d len %x tx_len %x\n",wsm_tx->id,hw_priv->wsm_tx_seq,wsm_tx->len,tx_len);
					//	frame_hexdump("atbm_bh wsm mesg",wsm_tx,10);	
						wsm_alloc_tx_buffer(hw_priv);
...
#if 0
		/*wait fw_ready and then start the bh process*/
		{
			reset_timeout = HZ;
			do
			{
				reset_timeout = wait_event_interruptible_timeout(hw_priv->bh_wq, ({
					fw_loading = atomic_read(&hw_priv->fw_reloading);
					rx = atomic_read(&hw_priv->bh_rx);
					tx = atomic_read(&hw_priv->bh_tx);
					(!fw_loading || rx || tx || hw_priv->bh_error);
				}), reset_timeout);

				if(fw_loading)
				{
					atomic_xchg(&hw_priv->bh_rx, 0);//drop current command
					atomic_xchg(&hw_priv->bh_tx, 0);
					reset_timeout = 0;
				}

				reset_timeout =  reset_timeout ? 0 : HZ;
			}
			while(reset_timeout&&(hw_priv->bh_error == 0));
			if(atomic_read(&hw_priv->reset_conter)>3)
			{
				goto bh_stop;
			}
			if(hw_priv->bh_error == 1)
			{
				atomic_xchg(&hw_priv->reset_flag, 0);
				atomic_xchg(&hw_priv->fw_reloading, 0);
				goto atbm_rest;
			}
		}
#endif
...
txrx:
		...
		if (rx) {
rx:
			while ((skb_rx = skb_dequeue(&hw_priv->rx_frame_queue)) != NULL) {
				u8 *data;
				data = skb_rx->data;
				wsm = (struct wsm_hdr *)data;
				//frame_hexdump("dump sdio wsm rx ->",wsm,32);
