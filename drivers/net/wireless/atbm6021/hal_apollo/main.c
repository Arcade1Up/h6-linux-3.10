...
//			wsm_unlock_tx(hw_priv);
	...
	*/
	if(
		test_bit(WSM_SET_CHANTYPE_FLAGS__CHANNUM_CHANGE,&hw_priv->change_bit)
		||
		test_bit(WSM_SET_CHANTYPE_FLAGS__CHANTYPE_CHANGE,&hw_priv->change_bit)
	  )
	{
		/*
		...
		*/
		...
		clear_bit(WSM_SET_CHANTYPE_FLAGS__CHANNUM_CHANGE,&hw_priv->change_bit);
		clear_bit(WSM_SET_CHANTYPE_FLAGS__CHANTYPE_CHANGE,&hw_priv->change_bit);		
	...
	*/
	...
	{
		if(!test_bit(WSM_SET_CHANTYPE_FLAGS__TOOL_SET_FORCE,&hw_priv->change_bit))
		{
	...
	else
	{
		...
		{			
			...
			if(test_bit(WSM_SET_CHANTYPE_FLAGS__CHANGE_PHY_CHTYPE,&hw_priv->change_bit))
			{
				action = notifyPeerMax;
				clear_bit(WSM_SET_CHANTYPE_FLAGS__CHANGE_PHY_CHTYPE,&hw_priv->change_bit);
			}
		}
	}
...
#define CCA_CHANGE_TO_20M_a  atomic_read(&hw_priv->chw_sw_20M_level)
	...
	*/
	if(can_get == 0)	{
		...
		if(test_bit(WSM_SET_CHANTYPE_FLAGS__CCA_LEVEL_CHANGE,&hw_priv->change_bit))	{
			printk("%s:cca_interval_change\n",__func__);
			clear_bit(WSM_SET_CHANTYPE_FLAGS__CCA_LEVEL_CHANGE,&hw_priv->change_bit);
...
static void atbm_skb_buff_queue(struct atbm_ba_tid_params *tid_params,u8 index,struct sk_buff *skb)
{
	...
	WARN_ON(tid_params->wind_size<tid_params->skb_buffed);
 	//__skb_queue_tail(&tid_params->header, skb);
}
static void atbm_skb_buff_dequeue(struct atbm_common *hw_priv,struct atbm_ba_tid_params *tid_params,u8 index,bool uplayer)
{
	...
	//__skb_dequeue(&tid_params->header)
}
...
struct ieee80211_hw *atbm_init_common(size_t hw_priv_data_len)
{
	...
	hw_priv->txrx_workqueue= create_singlethread_workqueue("txrx_workqueue");
#endif
#endif //#ifdef USB_BUS
	...
	skb_queue_head_init(&hw_priv->rx_frame_queue);
	skb_queue_head_init(&hw_priv->rx_frame_free);
#ifdef ATBM_SUPPORT_SMARTCONFIG
...
//	INIT_WORK(&hw_priv->chantype_change_work, atbm_channel_type_change_work);
	...
	clear_bit(WSM_SET_CHANTYPE_FLAGS__CHANNUM_CHANGE,&hw_priv->change_bit);
	clear_bit(WSM_SET_CHANTYPE_FLAGS__CHANTYPE_CHANGE,&hw_priv->change_bit);	
	clear_bit(WSM_SET_CHANTYPE_FLAGS__TOOL_SET_FORCE,&hw_priv->change_bit);
...
int atbm_register_common(struct ieee80211_hw *dev)
{
	...
	atbm_debug_init_common(hw_priv);
...
void atbm_unregister_common(struct ieee80211_hw *dev)
{
	...
	destroy_workqueue(hw_priv->txrx_workqueue);
	hw_priv->txrx_workqueue = NULL;
#endif
#endif
...
void atbm_set_40M(struct atbm_common *hw_priv,u32 params)
{
	...
	if((params == ATBM_40M_FREE)&&
		test_bit(WSM_SET_CHANTYPE_FLAGS__TOOL_SET_FORCE,&hw_priv->change_bit))
	{
	...
	{
		set_bit(WSM_SET_CHANTYPE_FLAGS__TOOL_SET_FORCE,&hw_priv->change_bit);
	}
...
void atbm_set_cca(struct atbm_common *hw_priv,u8 *buff)
{
		...
		(atomic_read(&hw_priv->cca_interval_ms) != patbm_cca_val->cca_interval))
	{
		...
		set_bit(WSM_SET_CHANTYPE_FLAGS__CCA_LEVEL_CHANGE,&hw_priv->change_bit);
	}
