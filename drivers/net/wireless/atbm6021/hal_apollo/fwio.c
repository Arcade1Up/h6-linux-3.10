...
void atbm_release_firmware(void)
{
	...
	if(atbm_fw.fw_dccm)
	{
		kfree(atbm_fw.fw_dccm);
		atbm_fw.fw_dccm = NULL;
	}
	if(atbm_fw.fw_iccm);
	{
		kfree(atbm_fw.fw_iccm);
...
int atbm_set_firmare(struct firmware_altobeam *fw)
{
	...
	{
		atbm_fw.fw_iccm = kzalloc(atbm_fw.hdr.iccm_len,GFP_KERNEL);
	...
	{
		atbm_fw.fw_dccm= kzalloc(atbm_fw.hdr.dccm_len,GFP_KERNEL);
...
static int atbm_load_firmware_generic(struct atbm_common *priv, u8 *data,u32 size,u32 addr)
{
	...
	buf = kmalloc(DOWNLOAD_BLOCK_SIZE*2, GFP_KERNEL | GFP_DMA);
	if (!buf) {
