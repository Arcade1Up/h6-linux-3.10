...
//#endif
#endif
	ht_info = (u8 *)cfg80211_find_ie( ATBM_WLAN_EID_HT_INFORMATION, ies, ies_len);
        if (ht_info) {
		/* Enable RIFS*/
		ht_info[3] |= 8;
        }
	erp_inf = (u8 *)cfg80211_find_ie(ATBM_WLAN_EID_ERP_INFO, ies, ies_len);
	if (erp_inf) {
	...
	if (priv->hidden_ssid) {
		...
		ssid_ie = (u8 *)cfg80211_find_ie(ATBM_WLAN_EID_SSID, ies, ies_len);
		WARN_ON(!ssid_ie);
		ssid_len = ssid_ie[1];
		if (ssid_len) {
...
static int atbm_upload_proberesp(struct atbm_vif *priv)
{
	...
	if (priv->hidden_ssid) {
		...
		ssid_ie = (u8*)cfg80211_find_ie(ATBM_WLAN_EID_SSID,
...
static int atbm_start_ap(struct atbm_vif *priv)
{
	...
	ssidie = cfg80211_find_ie(ATBM_WLAN_EID_SSID, skb->data + offset,
				  skb->len - offset);
