...
static bool  ieee80211_parase_p2p_action_attrs(u8* ie_start,size_t ie_len,struct atbm_p2p_message *p2p_msg)
{
	u8* p2p_ie = NULL;
	u8* attr_start = NULL;
	u16 attr_len = 0;
	...
	while(pos<end)
	{
		p2p_ie = atbm_ieee80211_find_p2p_ie(ie_start,ie_len);
...
static void ieee80211_change_p2p_channel_list_to_special_channel(u8* channel_list,u8 channel_num)
{
	u8 *pos, *end;
	u8 number_channel = 0;
	u8 index = 0;
...
bool atbm_parase_p2p_action_frame(struct atbm_vif *priv,struct sk_buff *skb,bool tx)
{
	...
	int len = skb->len;
	u8* p2p_data = NULL;
	size_t p2p_data_len = 0;
	size_t p2p_check_offs = 0;
	...
	printk(KERN_ERR "%s:operating_channel(%p),txrx(%d)\n",__func__,p2p_msg.operating_channel,(int)tx);
	...
	while(p2p_msg.operating_channel){
		...
		printk(KERN_ERR "%s:chan_mode(%x),operating_channel_combination(%d)\n",__func__,(int)combination,
		...
		*/
		if(p2p_data[0] == ATBM_P2P_INVITATION_REQ)
			atomic_set(&hw_priv->p2p_oper_channel,0);
		printk(KERN_ERR "%s:tx(%d),action(%d),operating_channel_num(%d),p2p_oper_channel(%d)\n",
...
change_channel_and_list:		
		...
		{
			ieee80211_change_p2p_channel_list_to_special_channel(p2p_msg.channel_list,local_channel);
		}

		if(p2p_data[0] != ATBM_P2P_INVITATION_RESP)
			break;
set_oper_channel:
		printk(KERN_ERR "%s p2p_oper_channel(%d)\n",__func__,local_channel);
	...
	while(tx == false)
	{
		...
		*/
		...
		printk(KERN_ERR "%s:intended_addr(%pM),own_addr(%pM)\n",__func__,p2p_msg.intended_addr+3,priv->vif->addr);
		break;
	}
