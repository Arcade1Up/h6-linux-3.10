...
#define ATBM_APOLLO_MAX_STA_IN_AP_MODE	(5)
#define ATBM_APOLLO_MAX_REQUEUE_ATTEMPTS	(5)
#define ATBM_APOLLO_LINK_ID_UNMAPPED		(15)

#define ATBM_APOLLO_MAX_TID			(8)

#define ATBM_APOLLO_TX_BLOCK_ACK_ENABLED_FOR_ALL_TID         (0x3F)
#define ATBM_APOLLO_RX_BLOCK_ACK_ENABLED_FOR_ALL_TID         (0x3F)
...
struct atbm_common {
	...
	* after the mechanism is clear */
	...
	#else
	struct workqueue_struct		*txrx_workqueue;
...
{
	return  (struct atbm_vif *)vif->drv_priv;
}
...
for (									\
	_i = 0;								\
	_priv = ATBM_HW_VIF_GET(_hw_priv->vif_list[_i]) ? 				\
	ABwifi_get_vif_from_ieee80211(ATBM_HW_VIF_GET(_hw_priv->vif_list[_i])) : NULL,	\
	_i < ATBM_WIFI_MAX_VIFS;						\
	_i++								\
)
...
#define skb_move_to(src,des,tskb)		\
			while((tskb = __skb_dequeue(src)))	\
				__skb_queue_tail(des, tskb)
