                                   ...
                                   unsigned int write_index)
{
        if (hif_pci_war1) {
              ...
              /*  indicator_addr = targid + ctrl_addr + DST_WATERMARK_ADDRESS; */
		        indicator_addr = targid;

                if (!war1_allow_sleep && ctrl_addr == CE_BASE_ADDRESS(CDC_WAR_DATA_CE)) {
                        A_PCI_WRITE32(indicator_addr, ctrl_addr + DST_WATERMARK_ADDRESS,
                                    (CDC_WAR_MAGIC_STR | write_index));
                } else {
                        unsigned long irq_flags;
                        local_irq_save(irq_flags);
                        A_PCI_WRITE32(indicator_addr, ctrl_addr + DST_WATERMARK_ADDRESS, 1);

                        /*
                         * PCIE write waits for ACK in IPQ8K, there is no
                         * need to read back value.
                         */
                        (void)A_PCI_READ32(indicator_addr, ctrl_addr + DST_WATERMARK_ADDRESS);
                        (void)A_PCI_READ32(indicator_addr, ctrl_addr + DST_WATERMARK_ADDRESS); /* conservative */

                        CE_SRC_RING_WRITE_IDX_SET(targid,
                                                  ctrl_addr,
                                                  write_index);

                        A_PCI_WRITE32(indicator_addr, ctrl_addr + DST_WATERMARK_ADDRESS, 0);
