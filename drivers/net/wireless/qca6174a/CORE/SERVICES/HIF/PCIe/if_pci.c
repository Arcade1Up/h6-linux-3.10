...
//	    pr_alert("BUG(%s): Clear PCI interrupt failed. tmp=%x, mem=%x\n", __func__, tmp, (unsigned int)sc->mem);
        pr_alert("BUG(%s): Clear PCI interrupt failed. tmp=%x\n", __func__, tmp);
    ...
    while(!hif_pci_targ_is_awake(sc, sc->mem)) {
        if(timeout_count >= PCIE_WAKE_TIMEOUT) {
            printk(KERN_ERR "Target cannot be woken up! "
                ...
                A_PCI_READ32(sc->mem, PCIE_LOCAL_BASE_ADDRESS + RTC_STATE_ADDRESS),
		A_PCI_READ32(sc->mem, PCIE_LOCAL_BASE_ADDRESS + PCIE_SOC_WAKE_ADDRESS));
            return -EACCES;
        }
...
//	    pr_alert("BUG(%s): Enable PCI interrupt failed. tmp=%x, mem=%x\n", __func__, tmp, (unsigned int)sc->mem);
        pr_alert("BUG(%s): Enable PCI interrupt failed. tmp=%x\n", __func__, tmp);
        print_config_soc_reg(sc);
	    //VOS_BUG(0);
	} 
...
//  printk(KERN_ERR "revision_id=%04x\n", revision_id);
...
hif_pci_configure(struct hif_pci_softc *sc, hif_handle_t *hif_hdl)
{
	...
	printk("%s: line = %d, rv = %d\n", __func__, __LINE__, rv);
#else
        rv = pci_enable_msi(sc->pdev);
	...
	if (rv == 0) { /* successfully allocated all MSI interrupts */
		...
		printk("%s: line = %d, msi irq num1 = %d\n", __func__, __LINE__, sc->pdev->irq+MSI_ASSIGN_FW);
		...
		for (i=MSI_ASSIGN_CE_INITIAL; i<=MSI_ASSIGN_CE_MAX; i++) {
			printk("%s: line = %d, msi irq num2 = %d\n", __func__, __LINE__, sc->pdev->irq+i);
    ...
    if ( num_msi_desired <= 1) {
	     ...
	     */
		printk("%s: line = %d, msi irq num3 = %d\n", __func__, __LINE__, sc->pdev->irq);
...
//      pr_alert("BUG(%s): Clear PCI interrupt failed. tmp=%x, mem=%x\n", __func__, tmp, (unsigned int)sc->mem);
    ...
    if ((val & 0x000000ff) != 0x3) {
        printk(KERN_ERR "chenjianwen add *** pci_save_state Enter\n");
        pci_save_state(pdev);
        pci_disable_device(pdev);
...
skip:
    ...
    for(i = 0; i < 16; i++) {
		...
		printk(KERN_ERR "chenjianwen add *** hif_pci_suspend Reg0x%d = 0x%x\n", i * 4, val);
	}
#endif
out:
    return ret;
...
__hif_pci_resume(struct pci_dev *pdev, bool runtime_pm)
{
	...
	for(i = 0; i < 16; i++) {
		pci_read_config_dword(pdev, i * 4, &val);
		printk(KERN_ERR "chenjianwen add *** hif_pci_resume before Reg0x%d = 0x%x\n", i * 4, val);
	}
#endif
	...
	for(i = 0; i < 16; i++) {
		pci_read_config_dword(pdev, i * 4, &val);
		printk(KERN_ERR "chenjianwen add *** hif_pci_resume after Reg0x%d = 0x%x\n", i * 4, val);
	}
#endif
...
unsigned int sunxi_ioread32(void __iomem *addr, unsigned long offset)
{
	...
	page = sunxi_pcie_device_cutpage_config(PCIE_BARBASE_0, offset);
	data = ioread32(page.mem_base + page.offset);
	sunxi_pcie_cutpage_spin_unlock(flags);

	return data;
}

void sunxi_iowrite32(void __iomem *addr, unsigned long offset, u32 value)
{
	struct pci_page page;
	unsigned long flags = 0;
	...
	flags = sunxi_pcie_cutpage_spin_lock();
	page = sunxi_pcie_device_cutpage_config(PCIE_BARBASE_0, offset);
	iowrite32(value, page.mem_base + page.offset);
	sunxi_pcie_cutpage_spin_unlock(flags);
}
