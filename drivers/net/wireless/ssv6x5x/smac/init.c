...
int ssv6xxx_load_firmware(struct ssv_hw *sh)
{
    ...
    if (cfgfirmwarepath != NULL)
    {
        ...
        printk(KERN_INFO "1111111 Using firmware at %s\n", temp_path);
    }
    else if (sh->cfg.firmware_path[0] != 0x00)
    {
        ...
        ret = SMAC_LOAD_FW(sh,temp_path, 1);
        printk(KERN_INFO "222222222 Using firmware at %s\n", temp_path);
    }
    else
    {
...
int ssv6xxx_dev_probe(struct platform_device *pdev)
{
    ...
    int ret;
    printk(KERN_ERR"enter ssv6xxx_dev_probe\n");
    if (!pdev->dev.platform_data) {
        dev_err(&pdev->dev, "no platform data specified!\n");
        return -EINVAL;
    }
    printk(KERN_ERR"%s(): SSV6X5X device \"%s\" found !\n", __FUNCTION__, pdev->name);
#ifdef SSV_MAC80211
...
#endif
{
 int ret = 0;
    ...
    if (ret != 0) {
        printk(KERN_ERR"%s(): Failed to register rc algorithm.\n", __FUNCTION__);
  return ret;
    }
    printk(KERN_ERR"in ssv6xxx_init,begin register ssv6xxx_driver");
