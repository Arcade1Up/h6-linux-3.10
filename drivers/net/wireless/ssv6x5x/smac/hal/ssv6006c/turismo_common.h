...
#ifndef TU_COMMON_H
#define TU_COMMON_H 
#define SSV6006_TURISMOC_COMMON_CODE_VER "1.12"
...
extern u8 xtal_sx_cp_isel_wf[];
extern char *xtal_type[];
#ifndef COMMON_FOR_SMAC
...
#define GET_PABAND(_ch,_pa_band) \
do{ \
    if (_ch <=14){ \
        pa_band =0; \
    } else if (_ch < 36) { \
...
do { \
    if (_cal->cal_iq_done[BAND_5900] == true){ \
                                                                                            \
...
#define TU_RESTART_RF_PHY(_band,_cal) \
do { \
    LOAD_TURISMOC_RF_TABLE(_patch); \
    REG32_W(ADR_WIFI_PHY_COMMON_ENABLE_REG, 0); \
    LOAD_TURISMOC_PHY_TABLE; \
