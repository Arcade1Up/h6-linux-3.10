...
static void _restore_dpd(struct ssv_hw *sh, struct ssv6006_padpd *dpd)
{
    ...
    for (i = 0; i < (MAX_PADPD_TONE/2) ; i++) {
        switch (dpd->current_band){
            case BAND_2G:
                addr_am = padpd_am_table_2G[i].addr;
                addr_pm = padpd_pm_table_2G[i].addr;
                break;
            case BAND_5100:
                addr_am = padpd_am_table_5100[i].addr;
                addr_pm = padpd_pm_table_5100[i].addr;
                break;
            case BAND_5500:
                addr_am = padpd_am_table_5500[i].addr;
                addr_pm = padpd_pm_table_5500[i].addr;
                break;
            case BAND_5700:
                addr_am = padpd_am_table_5700[i].addr;
                addr_pm = padpd_pm_table_5700[i].addr;
                break;
            case BAND_5900:
                addr_am = padpd_am_table_5900[i].addr;
                addr_pm = padpd_pm_table_5900[i].addr;
                break;
            default:
                addr_am = padpd_am_table_2G[i].addr;
                addr_pm = padpd_pm_table_2G[i].addr;
...
{
    if (sh->cal.cal_iq_done[BAND_5900] == true){
...
static void single_band_spur_patch(struct ssv_hw *sh, int xtal){
    ...
    , ssv_cabrio_reg *rf_tbl, ssv_cabrio_reg *phy_tbl)
{
    ...
    single_band_spur_patch(sh, patch.xtal);
                ...
                if (sc->dpd.dpd_done[pa_band]){
                    ...
                    switch (pa_band){
                        case 0:
                            ...
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_am_table_2G[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                        ...
                        case 1:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_am:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_am_table_5100[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "%03d %03d ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                        ...
                        case 2:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_am:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_am_table_5500[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "%03d %03d ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                        ...
                        case 3:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_am:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_am_table_5700[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "%03d %03d ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                        ...
                        case 4:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_am:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_am_table_5900[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "%03d %03d ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                    ...
                    switch (pa_band){
                        case 0:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_pm:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_pm_table_2G[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "0x%04x 0x%04x ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                            }
                            break;
                        case 1:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_pm:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_pm_table_5100[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "0x%04x 0x%04x ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                        ...
                        case 2:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_pm:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_pm_table_5500[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "0x%04x 0x%04x ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                        ...
                        case 3:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_pm:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_pm_table_5700[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "0x%04x 0x%04x ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
                        ...
                        case 4:
                            snprintf_res(cmd_data,"\n\t\tread back bank%d am_pm:", pa_band);
                            for (i = 0 ; i < MAX_PADPD_TONE/2; i ++){
                                regval = REG32_R(padpd_pm_table_5900[i].addr);
                                if (i %4 == 0)
                                    snprintf_res(cmd_data,"\n\t\t");
                                    snprintf_res(cmd_data, "0x%04x 0x%04x ", regval& 0xffff,
                                    (regval >>16) & 0xffff);
...
void ssv6006_turismoC_update_rf_pwr(struct ssv_softc *sc){
    ...
    gt_pa_band = _get_pa_band(sc->gt_channel);
