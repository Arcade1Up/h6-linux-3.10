...
)
{
	...
	odm_memory_set(p_dm_odm, &(p_dbg->num_qry_pkt_sc_20m[0]), 0, (LOW_BW_RATE_NUM * 2));
	
	p_dbg->ht_pkt_not_zero = false;
	p_dbg->low_bw_20_occur = false;
	
#if	ODM_IC_11AC_SERIES_SUPPORT
	odm_memory_set(p_dm_odm, &(p_dbg->num_qry_vht_pkt[0]), 0, (VHT_RATE_NUM * 2));
	odm_memory_set(p_dm_odm, &(p_dbg->num_qry_pkt_sc_40m[0]), 0, (LOW_BW_RATE_NUM * 2));
	
	p_dbg->vht_pkt_not_zero = false;
	p_dbg->low_bw_40_occur = false;
#endif
}
...
)
{
	...
	if (p_dbg->ht_pkt_not_zero) {
		...
		if (p_dbg->low_bw_20_occur) {

			for (i = 0; i < rate_num; i++) {
				
				rate_ss_shift = (i << 3);
				
				PHYDM_DBG(p_dm, ODM_COMP_COMMON, ("* [Low BW 20M] HT MCS[%d :%d ] = {%d, %d, %d, %d, %d, %d, %d, %d}\n",
					(rate_ss_shift), (rate_ss_shift+7),
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 0], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 1],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 2], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 3],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 4], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 5],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 6], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 7]));
			}
		}	
	}
	...
	if (p_dbg->vht_pkt_not_zero){
		...
		if (p_dbg->low_bw_20_occur) {
			for (i = 0; i < rate_num; i++) {
				...
				PHYDM_DBG(p_dm, ODM_COMP_COMMON, ("*[Low BW 20M] VHT-%d ss MCS[0:9] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d}\n",
					(i + 1),
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 0], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 1],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 2], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 3],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 4], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 5],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 6], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 7],
					p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 8], p_dbg->num_qry_pkt_sc_20m[rate_ss_shift + 9]));
			}
		}

		if (p_dbg->low_bw_40_occur) {
			for (i = 0; i < rate_num; i++) {
				rate_ss_shift = 10 * i;
				
				PHYDM_DBG(p_dm, ODM_COMP_COMMON, ("*[Low BW 40M] VHT-%d ss MCS[0:9] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d}\n",
					(i + 1),
					p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 0], p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 1],
					p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 2], p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 3],
					p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 4], p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 5],
					p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 6], p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 7],
					p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 8], p_dbg->num_qry_pkt_sc_40m[rate_ss_shift + 9]));
			}
		}
	}
#endif
...
)
{
	...
	if (p_dm_odm->is_linked) {
		...
		phydm_get_phy_statistic(p_dm_odm);
