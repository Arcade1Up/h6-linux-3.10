	...
	if (date_rate <= ODM_RATE54M) {
		...
		if (p_phydm->support_ic_type & ODM_IC_PHY_STATUE_NEW_TYPE) {
			...
			if ((bw_idx == *(p_phydm->p_band_width))) {
				...
				p_phydm->phy_dbg_info.num_qry_pkt_sc_20m[date_rate - ODM_RATEMCS0]++;
				p_phydm->phy_dbg_info.low_bw_20_occur = true;
			}
		} else {
			p_phydm->phy_dbg_info.num_qry_ht_pkt[date_rate - ODM_RATEMCS0]++;
		}
		...
		#endif
		{
			...
			if (p_phydm->support_ic_type & ODM_IC_PHY_STATUE_NEW_TYPE) {
				if ((bw_idx == *(p_phydm->p_band_width))) {
					...
					p_phydm->phy_dbg_info.num_qry_pkt_sc_20m[date_rate - ODM_RATEVHTSS1MCS0]++;
					p_phydm->phy_dbg_info.low_bw_20_occur = true;
				} else /*if (bw_idx == CHANNEL_WIDTH_40)*/ {
					p_phydm->phy_dbg_info.num_qry_pkt_sc_40m[date_rate - ODM_RATEVHTSS1MCS0]++;
					p_phydm->phy_dbg_info.low_bw_40_occur = true;
				}
			} else {
				p_phydm->phy_dbg_info.num_qry_vht_pkt[date_rate - ODM_RATEVHTSS1MCS0]++;
			}
...
)
{
	...
	if (p_pktinfo->is_packet_match_bssid) {
		...
		phydm_rx_statistic_cal(p_dm_odm, p_phy_info, p_phy_status, p_pktinfo);
	}
...
#if (RTL8821C_SUPPORT == 1)
				if (p_dm_odm->support_ic_type & ODM_RTL8821C)
					rx_power = phydm_cck_rssi_8821c(p_dm_odm, lna_idx, vga_idx);
#endif
