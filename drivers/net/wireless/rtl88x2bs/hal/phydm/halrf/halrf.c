...
)
{
	...
	if (is_check)
		p_iqk_info->is_hwtx = (boolean)odm_get_bb_reg(p_dm_odm, 0xb00, BIT(8));
	else {
		if (p_iqk_info->is_hwtx) {
			tmp_b04 = odm_read_4byte(p_dm_odm, 0xb04);
			odm_set_bb_reg(p_dm_odm, 0xb04, BIT(3) | BIT (2), 0x0);
#if 0
			tmp = odm_read_4byte(p_dm, 0xb00);
			odm_write_4byte(p_dm, 0xb00, tmp | BIT(24));
			odm_write_4byte(p_dm, 0xb00, tmp & (~BIT(24)));
			odm_write_4byte(p_dm, 0xb00, tmp | BIT(24));
#endif
			odm_write_4byte(p_dm_odm, 0xb04, tmp_b04);
		}
	}
}
