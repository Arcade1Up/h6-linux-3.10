					...
					*/
					if ((p_dm_fat_table->main_ant_cnt[i] != 0) || (p_dm_fat_table->aux_ant_cnt[i] != 0)) {
						...
						if (p_dm_swat_table->pre_antenna == MAIN_ANT) {
							target_ant = ((aux_ant_cnt > main_ant_cnt) && (aux_rssi >= main_rssi)) ? AUX_ANT : p_dm_swat_table->pre_antenna;
						} else {
							target_ant = ((main_ant_cnt > aux_ant_cnt) && (main_rssi >= aux_rssi)) ? MAIN_ANT : p_dm_swat_table->pre_antenna;
						}
...
)
{
	...
	if (p_dm_odm->antdiv_select == 1)
		...
		if (p_dm_odm->consecutive_idlel_time >= 10) {
			p_dm_odm->ant_type = ODM_FIX_MAIN_ANT;
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("[AntDiv: OFF] No TP case, consecutive_idlel_time=((%d))\n", p_dm_odm->consecutive_idlel_time));
		}
		#endif
	}
	...
	/* 2 [--8821C---] */
		...
		if (!p_dm_odm->is_bt_continuous_turn) {
			p_dm_odm->ant_div_type = S0S1_SW_ANTDIV;
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("is_bt_continuous_turn = ((%d))   ==> SW AntDiv\n", p_dm_odm->is_bt_continuous_turn));

		} else {
			p_dm_odm->ant_div_type = CG_TRX_HW_ANTDIV;
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("is_bt_continuous_turn = ((%d))   ==> HW AntDiv\n", p_dm_odm->is_bt_continuous_turn));
		}
