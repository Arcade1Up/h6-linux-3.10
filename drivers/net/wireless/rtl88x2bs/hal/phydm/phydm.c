...
/* JJ ADD 20161014 */
	...
	if (p_dm_odm->support_ic_type & ODM_RTL8821C) {
		...
		if (p_dm_odm->cck_new_agc == 0 && p_dm_odm->default_rf_set_8821c == SWITCH_TO_BTG)
			p_dm_odm->cck_agc_report_type = 1;
	}
#endif

/* JJ ADD 20161014 */
#if ((RTL8723D_SUPPORT == 1) || (RTL8822B_SUPPORT == 1) || (RTL8197F_SUPPORT == 1) || (RTL8710B_SUPPORT == 1))
	if (p_dm_odm->support_ic_type & (ODM_RTL8723D | ODM_RTL8822B | ODM_RTL8197F | ODM_RTL8710B))
		p_dm_odm->cck_new_agc = odm_get_bb_reg(p_dm_odm, 0xa9c, BIT(17)) ? true : false;          /*1: new agc  0: old agc*/
	else
#endif
...
)
{
	...
	case	ODM_CMNINFO_BT_CONTINUOUS_TURN:
		p_dm_odm->is_bt_continuous_turn = (boolean)value;
		break;
...
)
{
	...
	u32	bb_regf0 = odm_get_bb_reg(p_dm_odm, 0xf0, MASKDWORD);

	if (!(p_dm_odm->support_ic_type & ODM_RECEIVER_BLOCKING_SUPPORT))
		return;

	if ((p_dm_odm->support_ic_type & ODM_RTL8188E && ((bb_regf0 & 0xf000) >> 12) < 8) ||
		p_dm_odm->support_ic_type & ODM_RTL8192E) {/*8188E_T version*/
		if (p_dm_odm->consecutive_idlel_time > 10 && *p_dm_odm->p_mp_mode == false && p_dm_odm->adaptivity_enable == true) {
			if ((bw == ODM_BW20M) && (channel == 1)) {
				phydm_nbi_setting(p_dm_odm, NBI_ENABLE, channel, 20, 2410, PHYDM_DONT_CARE);
				p_dm_odm->is_receiver_blocking_en = true;
			} else if ((bw == ODM_BW20M) && (channel == 13)) {
				phydm_nbi_setting(p_dm_odm, NBI_ENABLE, channel, 20, 2473, PHYDM_DONT_CARE);
				p_dm_odm->is_receiver_blocking_en = true;
			} else if (p_dm_odm->is_receiver_blocking_en && channel != 1 && channel != 13) {
				phydm_nbi_enable(p_dm_odm, NBI_DISABLE);
	...
	} else if ((p_dm_odm->support_ic_type & ODM_RTL8188E && ((bb_regf0 & 0xf000) >> 12) >= 8)) {/*8188E_S version*/
		if (p_dm_odm->consecutive_idlel_time > 10 && *p_dm_odm->p_mp_mode == false && p_dm_odm->adaptivity_enable == true) {
			if ((bw == ODM_BW20M) && (channel == 13)) {
				phydm_nbi_setting(p_dm_odm, NBI_ENABLE, channel, 20, 2473, PHYDM_DONT_CARE);
				p_dm_odm->is_receiver_blocking_en = true;
			} else if (p_dm_odm->is_receiver_blocking_en && channel != 13) {
				phydm_nbi_enable(p_dm_odm, NBI_DISABLE);
				odm_set_bb_reg(p_dm_odm, 0xc40, 0x1f000000, 0x1f);
				p_dm_odm->is_receiver_blocking_en = false;
			}
...
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		for (i = ODM_RF_PATH_A; i <= ODM_RF_PATH_B; i++) {
			odm_set_rf_reg(p_dm, (enum odm_rf_radio_path_e)i, 0xEF, BIT(19), 0x1); /*RF mode table write enable*/
			odm_set_rf_reg(p_dm, (enum odm_rf_radio_path_e)i, 0x33, 0xF, 3); /*Select RX mode*/
			odm_set_rf_reg(p_dm, (enum odm_rf_radio_path_e)i, 0x3E, 0xfffff, 0x00036); /*Set Table data*/
			odm_set_rf_reg(p_dm, (enum odm_rf_radio_path_e)i, 0x3F, 0xfffff, 0x5AFCE); /*Set Table data*/
			odm_set_rf_reg(p_dm, (enum odm_rf_radio_path_e)i, 0xEF, BIT(19), 0x0); /*RF mode table write disable*/
		}
	}
	#endif
#endif
...
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(p_dm, 0x808, 0x3ffff00, 0);
		odm_set_bb_reg(p_dm, 0x9ac, 0x1fff, 0);
		odm_set_bb_reg(p_dm, 0x9ac, BIT(13), 1);
	}
	#endif
#endif
}
...
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		phydm_iq_gen_en(p_dm);

		#if 0/*CONFIG_PHYDM_DEBUG_FUNCTION*/
		if (phydm_api_trx_mode(p_dm,  (enum odm_rf_path_e)3,  (enum odm_rf_path_e)3, true) == false)
			return;
		#else
		...
		phydm_dis_cdd(p_dm);
		odm_set_bb_reg(p_dm, 0x195c, MASKDWORD, 0x40000);	/*Set Q matrix r_v11 =1*/
		phydm_pathb_q_matrix_rotate(p_dm, 0);
		odm_set_bb_reg(p_dm, 0x191c, BIT(7), 1);		/*Set Q matrix enable*/
	}
	#endif
#endif
}
...
)
{
	...
	if (phase_idx >= 12) {
		PHYDM_DBG(p_dm, ODM_COMP_API, ("Phase Set Error: %d\n", phase_idx));
		return;
	}
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(p_dm, 0x1954, 0xffffff, phase_table_0[phase_idx]); /*Set Q matrix r_v21*/
		odm_set_bb_reg(p_dm, 0x1950, 0xffffff, phase_table_1[phase_idx]);
	}
	#endif
#endif
}
...
phydm_set_bb_txagc_offset(
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		if (power_offset > -16 || power_offset < 15) {
			...
			dbg_print("[Warning] TX AGC Offset Setting error!");
		}
	}
	#endif
	...
	if (dm->support_ic_type & ODM_IC_11N_SERIES) {
		if (power_offset > -8 || power_offset < 7) {
			...
			PHYDM_DBG(dm, ODM_COMP_API, ("Reg_idx =0x%x\n", power_idx));
			odm_set_bb_reg(dm, 0x80c, 0x1f00, power_idx);	/*r_txagc_offset_a*/
			odm_set_bb_reg(dm, 0x80c, 0x3e000, power_idx);/*r_txagc_offset_b*/
			set_success = true;
		} else {
			dbg_print("[Warning] TX AGC Offset Setting error!");
		}
