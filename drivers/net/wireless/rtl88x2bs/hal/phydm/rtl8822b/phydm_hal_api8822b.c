...
)
{
	if (channel <= 14) {
			/* signal source */
			odm_set_bb_reg(p_dm_odm, 0xcb0, 0xffffff, 0x777777);
			odm_set_bb_reg(p_dm_odm, 0xeb0, 0xffffff, 0x777777);
			odm_set_bb_reg(p_dm_odm, 0xcb4, MASKBYTE1, 0x77);
			odm_set_bb_reg(p_dm_odm, 0xeb4, MASKBYTE1, 0x77);
		
	} else if ((channel > 35) && (channel <= 64)) {
			/* signal source */
			odm_set_bb_reg(p_dm_odm, 0xcb0, 0xffffff, 0x777747);
			odm_set_bb_reg(p_dm_odm, 0xeb0, 0xffffff, 0x777747);
			odm_set_bb_reg(p_dm_odm, 0xcb4, MASKBYTE0, 0x57);
			odm_set_bb_reg(p_dm_odm, 0xeb4, MASKBYTE0, 0x57);
				
	} else if (channel > 64) {
			/* signal source */
			odm_set_bb_reg(p_dm_odm, 0xcb0, 0xffffff, 0x777747);
			odm_set_bb_reg(p_dm_odm, 0xeb0, 0xffffff, 0x777747);
			odm_set_bb_reg(p_dm_odm, 0xcb4, MASKBYTE0, 0x75);
			odm_set_bb_reg(p_dm_odm, 0xeb4, MASKBYTE0, 0x75);

	} else
			return;
	
	/* inverse or not */
	odm_set_bb_reg(p_dm_odm, 0xcbc, 0x3f, 0x0);
	odm_set_bb_reg(p_dm_odm, 0xcbc, (BIT(11) | BIT(10) | BIT(9) | BIT(8)), 0x0);
	odm_set_bb_reg(p_dm_odm, 0xebc, 0x3f, 0x0);
	odm_set_bb_reg(p_dm_odm, 0xebc, (BIT(11) | BIT(10) | BIT(9) | BIT(8)), 0x0);
...
)
{
	...
	if ((p_dm_odm->rfe_type == 4) || (p_dm_odm->rfe_type == 11)) {
	...
	} else if ((p_dm_odm->rfe_type == 0) || (p_dm_odm->rfe_type == 3) || (p_dm_odm->rfe_type == 5) || (p_dm_odm->rfe_type == 8) || (p_dm_odm->rfe_type == 10) || (p_dm_odm->rfe_type == 12) || (p_dm_odm->rfe_type == 13) || (p_dm_odm->rfe_type == 14) || (p_dm_odm->rfe_type == 16)) {
		/* iFEM */
		if (channel <= 14) {
			/* signal source */
			odm_set_bb_reg(p_dm_odm, 0xcb0, (MASKBYTE2 | MASKLWORD), 0x745774);
		...
		phydm_8822b_type15_rfe(p_dm_odm, channel);		
	}
...
)
{
	...
	if ((p_dm_odm->rfe_type == 3) || (p_dm_odm->rfe_type == 5) || (p_dm_odm->rfe_type == 12) || (p_dm_odm->rfe_type == 15) || (p_dm_odm->rfe_type == 16)) {
	...
	if ((p_dm_odm->rfe_type == 16) && (central_ch_8822b <= 14))
		odm_set_bb_reg(p_dm_odm, 0x8c8, BIT(3) | BIT (2) | BIT(1), 0x3);
...
)
{
	...
	if (idx <= 16) {
		...
		ODM_RT_TRACE(p_dm_odm, ODM_BB_DYNAMIC_PSDTOOL, ODM_DBG_LOUD, ("[Return Point] Idx Is Exceed, Not Support Dynamic Spur Detection and Eliminator\n"));
		return;
	}
	...
	if (s_donbi_a == true || s_donbi_b == true) {
		if (*p_dm_odm->p_band_width == ODM_BW20M) {
			...
			if (*p_dm_odm->p_channel == 155) {
				set_result_nbi = phydm_nbi_setting(p_dm_odm, NBI_ENABLE, *p_dm_odm->p_channel, 80, 5760, PHYDM_DONT_CARE);
			} else if (*p_dm_odm->p_channel == 58) {
				set_result_nbi = phydm_nbi_setting(p_dm_odm, NBI_ENABLE, *p_dm_odm->p_channel, 80, 5280, PHYDM_DONT_CARE);
			} else if (*p_dm_odm->p_channel == 122) {
				set_result_nbi = phydm_nbi_setting(p_dm_odm, NBI_ENABLE, *p_dm_odm->p_channel, 80, 5600, PHYDM_DONT_CARE);
			} else
				set_result_nbi = SET_NO_NEED;
		} else
			set_result_nbi = SET_NO_NEED;
...
)
{
	...
	if (central_ch <= 14) {
		...
		if ((reg_8 == 0x1) && (!((p_dm_odm->rfe_type == 3) || (p_dm_odm->rfe_type == 5)))) {
			...
			odm_set_bb_reg(p_dm_odm, 0x8d8, BIT(19), 0x0);
...
)
{
	...
	if (central_ch <= 14) {
		...
		} else if ((p_dm_odm->rfe_type == 15) || (p_dm_odm->rfe_type == 16)) {
			if (p_dm_odm->brxagcswitch == true) {
				phydm_rxagc_switch_8822b(p_dm_odm, true);
			} else {
				phydm_rxagc_switch_8822b(p_dm_odm, false);
			}
		}
