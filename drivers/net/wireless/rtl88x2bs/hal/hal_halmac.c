...
static inline enum _HALMAC_RF_TYPE _rf_type_drv2halmac(
				enum _RT_RF_TYPE_DEFINITION rf_drv)
{
...
static inline enum odm_rf_type_e _rf_type_halmac2phydm(
				enum _HALMAC_RF_TYPE rf_mac)
{
	enum odm_rf_type_e rf_phydm;
...
static inline void _rf_type_to_ant_path(enum odm_rf_type_e rf, u8 *tx, u8 *rx)
{
	if (tx) {
		switch (rf) {
		case ODM_1T1R:
		...
		case ODM_2T3R:
		case ODM_2T4R:
			*tx = ODM_RF_A | ODM_RF_B;
			break;
		case ODM_3T3R:
		case ODM_3T4R:
			*tx = ODM_RF_A | ODM_RF_B | ODM_RF_C;
			break;
		case ODM_4T4R:
		default:
			*tx = ODM_RF_A | ODM_RF_B | ODM_RF_C | ODM_RF_D;
			break;
		}
	}
	...
	if (rx) {
		...
		case ODM_2T2R:
		case ODM_2T2R_GREEN:
			*rx = ODM_RF_A | ODM_RF_B;
			break;
		case ODM_2T3R:
		case ODM_3T3R:
			*rx = ODM_RF_A | ODM_RF_B | ODM_RF_C;
			break;
		case ODM_2T4R:
		case ODM_3T4R:
		case ODM_4T4R:
		default:
			*rx = ODM_RF_A | ODM_RF_B | ODM_RF_C | ODM_RF_D;
			break;
		}
	}
}
				     ...
				     struct _HALMAC_GENERAL_INFO *info)
{
	...
	hal = GET_HAL_DATA(a);
	rf_phydm = _rf_type_halmac2phydm(info->rf_type);
	_rf_type_to_ant_path(rf_phydm, &txpath, &rxpath);
...
#define GENERAL_INFO_REG_SET_TX_ANT_STATUS(buf, v)	SET_BITS_TO_LE_1BYTE(buf+4, 4, 4, v)
	...
	GENERAL_INFO_REG_SET_RF_TYPE(h2c, rf_phydm);
	GENERAL_INFO_REG_SET_CUT_VERSION(h2c, cut_phydm);
	GENERAL_INFO_REG_SET_RX_ANT_STATUS(h2c, rxpath);
	GENERAL_INFO_REG_SET_TX_ANT_STATUS(h2c, rxpath);
...
static int _send_general_info(struct dvobj_priv *d)
{
	...
	info.rf_type = _rf_type_drv2halmac((enum _RT_RF_TYPE_DEFINITION)val8);
...
static int download_fw(struct dvobj_priv *d, u8 *fw, u32 fwsize, u8 re_dl)
{
	...
	/* 4.1. Reset driver variables if needed */
	hal->LastHMEBoxNum = 0;
