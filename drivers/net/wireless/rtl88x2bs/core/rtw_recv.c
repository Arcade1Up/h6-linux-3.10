...
void count_rx_stats(_adapter *padapter, union recv_frame *prframe, struct sta_info *sta)
{
	...
	struct adaptive_soml	*p_dm_soml_table = &pDM_Odm->dm_soml_table;
#endif
...
#if 1
	if ((pattrib->data_rate >= ODM_RATEMCS0) && (pattrib->data_rate <= ODM_RATEMCS31)) {
		p_dm_soml_table->num_ht_bytes[pattrib->data_rate - ODM_RATEMCS0] += sz;
#if 0
		RTW_INFO("ht[%d] = %u\n", pattrib->data_rate - ODM_RATEMCS0,
				p_dm_soml_table->num_ht_bytes[pattrib->data_rate - ODM_RATEMCS0]);
#endif
	} else if ((pattrib->data_rate >= ODM_RATEVHTSS1MCS0) && (pattrib->data_rate <= ODM_RATEVHTSS4MCS9)) {
		p_dm_soml_table->num_vht_bytes[pattrib->data_rate - ODM_RATEVHTSS1MCS0] += sz;
#if 0
		RTW_INFO("vht[%d] = %u\n", pattrib->data_rate - ODM_RATEVHTSS1MCS0,
				p_dm_soml_table->num_vht_bytes[pattrib->data_rate - ODM_RATEVHTSS1MCS0]);
#endif
	}
#endif
