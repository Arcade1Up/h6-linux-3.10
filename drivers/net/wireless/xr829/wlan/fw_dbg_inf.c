		...
		char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (p_msg == NULL) {
		pos += sprintf(&buf[pos], "but not enough memory to show.\n");
		goto err;
	}
	...
	if (xradio_fw_dbg_request(hw_priv, p_msg, msg_req_size, msg_buf_size,
		...
		pos += sprintf(&buf[pos], "but cfm msg status error.\n");
		goto err;
	}

	fwd_priv.sys_config = *p_sys_cfg;

	pos += sprintf(&buf[pos], "frame trace: %08x, cmd trace: %08x, func trace: %08x\n"
		...
		char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (stored_num == 0x0) {
		pos += sprintf(&buf[pos], "catch none.\n");
		goto end;
	}

	pos += sprintf(&buf[pos], "idx|load|stat_time|proc_time|idle_time|"
	...
	for (num = 0; num < max_num; num++) {
		...
		pos += sprintf(&buf[pos], "%3d|%3d%%|%9d|%9d|%9d|%8d|%8d|%8d\n",
			idx, cpu_load,
	...
	const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	case 0x2:
		...
		pos += sprintf(&print_buf[pos], "idx");

		for (print_idx = 0x0; print_idx < 32; print_idx++) {
			pos += sprintf(&print_buf[pos], "|%s%02d",
				"evt", print_idx);
		}
		...
		for (idx = 0; idx < p_store->ind_write_idx; idx++) {
			...
			pos += sprintf(&print_buf[pos], "%3d", idx);

			for (print_idx = 0x0; print_idx < 32; print_idx++) {
				pos += sprintf(&print_buf[pos],"|%5d",
					p_sub[print_idx]);
			}
			xradio_dbg(XRADIO_DBG_ALWY, "%s\n", print_buf);
			pos = 0;
...
static int fw_dbg_sys_parse_func_trace(struct fwd_sys_func_trace *p_trace)
{
	...
	pos += sprintf(&buf[pos], "stage %02x:",p_trace->stage);

	for (i = 0; i < 8; i++) {
		if ((1 << i) & (p_trace->stage)) {
			pos += sprintf(&buf[pos], "%s ", place_name[i]);
		}
	}
					...
					struct fwd_soc_lpclk_stat *p_stat)
{
	...
	pos += sprintf(&buf[pos], "---lp clk statitics:\n"
		"tsf_diff_sum: %d \n"
		"tsf_diff_max: %d \n"
		"tsf_diff_cur: %d \n"
		"tsf_diff_cnt: %d \n"
	...
	pos += sprintf(&buf[pos], "---lp clk calculate:\n"
		"tsf_diff_avg(us): %d \n"
		"sleep time: %d\n"
...
{
	int pos = strlen(buf);
	pos += sprintf(&buf[pos], "pas statistics counter-----------\n");

	pos += sprintf(&buf[pos], "fiq count :%4d, rx event:%4d\n",
		p_stat->fiq_count, p_stat->rx_event_count);

	pos += sprintf(&buf[pos], "tx [write port:%4d][success:%3d][failure:%2d]\n",
		p_stat->write_port_count,
		p_stat->tx_success_count,
		p_stat->tx_failure_count);

	pos += sprintf(&buf[pos], "rx [stored :%4d][error:%3d][beacon :%2d][multicast: %4d]\n",
		...
		char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	pos += sprintf(&buf[pos], "rf_switch_ctrl: %08x\n\n", p_cfg->rf_switch_ctrl);
		...
		char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	pos += sprintf(&buf[pos], "on_off_ctrl: 0x%08x\n\n", p_cfg->on_off_ctrl);
