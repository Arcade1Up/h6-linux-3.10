...
void xradio_join_work(struct work_struct *work)
{
	...
	mutex_lock(&hw_priv->conf_mutex);
	{
		...
		join.PhyModeCfg.ModemFlags  = (hw_priv->channel->band ==
			IEEE80211_BAND_5GHZ) ? (MODEM_F_A_OFDM|MODEM_F_N_OFDM) :
		...
		if (bss_ht_info_ie != NULL) {
			...
			struct ieee80211_ht_info *bss_ht_info;
			struct ieee80211_supported_band *sband;

			bss_ht_info = (struct ieee80211_ht_info *)(bss_ht_info_ie + 2);
				...
				struct ieee80211_channel *chan)
{
	...
	struct wsm_start start = {
		...
		.PhyModeCfg.BandCfg = (chan->band == IEEE80211_BAND_5GHZ) ?
				WSM_PHY_BAND_5G : WSM_PHY_BAND_2_4G,
		.PhyModeCfg.ModemFlags = (chan->band == IEEE80211_BAND_5GHZ) ?
