...
static int update_hwinfo_buffer(struct xradio_common *hw_priv)
{
	/* check whether efues data is wirte to file already*/
	u32 hw_arry[64] = { 0 };
...
int __init xradio_core_entry(void)
{
	...
	hwinfo_proc_node = proc_create("hwinfo", 0666, hwinfo_proc_dir, &hwinfo_proc_op);
