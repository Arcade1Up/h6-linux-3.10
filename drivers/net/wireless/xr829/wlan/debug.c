...
#if (DBG_XRADIO_HIF)
u16 hif_test_rw = 0; /*0: nothing to do; 1: write only; 2: write and read*/
u16 hif_test_data_mode = 0; /* hif test data mode, such as 0x55, 0xff etc*/
u16 hif_test_data_len = 0; /* hif test data len, every data len pre round*/
u16 hif_test_data_round = 0;
u16 hif_test_oper_delta = 0; /* hif test operation delta time, give more time to analyze data tranx*/
#endif
...
//	memset(&epta_stat, 0, sizeof(epta_stat));
	...
	for(i=0; i<sizeof(epta_stat); i++){
		if(i % 4 == 0) seq_printf(seq, "%d:\t ", i );
		seq_printf(seq, "%2x ", *((u8*)&epta_stat+i));
		if((i+1) % 4 == 0) seq_printf(seq, "\n");
	}
	seq_printf(seq, "\n================\n");
#endif
	if (ret)
	...
	if(epta_stat.medium_state == 0) return 0;
	...
	for(i=0; i<11; i++) if(epta_stat_dbg_ctrl & (1<<i)) seq_printf(seq, "%10s ", req_type_str[i]);
	seq_printf(seq, "\n");
