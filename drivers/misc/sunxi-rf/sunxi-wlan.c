...
struct sunxi_wlan_platdata {
	int bus_index;
	int usb_index;
	struct regulator *wlan_power1;
	struct regulator *wlan_power2;
	...
	char *wlan_power_name1;
	char *wlan_power_name2;
...
static int sunxi_wlan_on(struct sunxi_wlan_platdata *data, bool on_off)
{
	...
	if (data->wlan_power_name1) {
		data->wlan_power1 = regulator_get(dev, data->wlan_power_name1);
		if (!IS_ERR(data->wlan_power1)) {
			if(on_off){
				ret = regulator_enable(data->wlan_power1);
				if (ret < 0){
					dev_err(dev, "regulator wlan_power1 enable failed\n");
					regulator_put(data->wlan_power1);
					return ret;
				}

				ret = regulator_get_voltage(data->wlan_power1);
				if (ret < 0){
					dev_err(dev, "regulator wlan_power1 get voltage failed\n");
					regulator_put(data->wlan_power1);
					return ret;
				}
				dev_info(dev, "check wlan wlan_power1 voltage: %d\n", ret);
			}else{
				ret = regulator_disable(data->wlan_power1);
				if (ret < 0){
					dev_err(dev, "regulator wlan_power1 disable failed\n");
					regulator_put(data->wlan_power1);
					return ret;
				}
			}
	...
	if (data->wlan_power_name2) {
		data->wlan_power2 = regulator_get(dev, data->wlan_power_name1);
		if (!IS_ERR(data->wlan_power2)) {
			if (on_off) {
				ret = regulator_enable(data->wlan_power2);
				if (ret < 0) {
					dev_err(dev, "regulator wlan_power2 enable failed\n");
					regulator_put(data->wlan_power2);
					return ret;
				}

				ret = regulator_get_voltage(data->wlan_power2);
				if (ret < 0) {
					dev_err(dev, "regulator wlan_power2 get voltage failed\n");
					regulator_put(data->wlan_power2);
					return ret;
				}
				dev_info(dev, "check wlan wlan_power2 voltage: %d\n", ret);
			} else {
				ret = regulator_disable(data->wlan_power2);
				if (ret < 0) {
					dev_err(dev, "regulator wlan_power2 disable failed\n");
					regulator_put(data->wlan_power2);
					return ret;
				}
			}
...
static int sunxi_wlan_probe(struct platform_device *pdev)
{
	...
	u32 val;
	const char *power1, *power2, *io_regulator;
	int ret = 0;
	int clk_gpio;
	char pin_name[32];
	...
	if (of_property_read_string(np, "wlan_power1", &power1)) {
		dev_warn(dev, "Missing wlan_power1.\n");
	} else{
		data->wlan_power_name1 = devm_kzalloc(dev, 64, GFP_KERNEL);
		if (!data->wlan_power_name1)
			return -ENOMEM;
		else
			strcpy(data->wlan_power_name1, power1);
	}
	dev_info(dev, "wlan_power_name1 (%s)\n", data->wlan_power_name1);

	if (of_property_read_string(np, "wlan_power2", &power2)) {
		dev_warn(dev, "Missing wlan_power2.\n");
	} else {
		data->wlan_power_name2 = devm_kzalloc(dev, 64, GFP_KERNEL);
		if (!data->wlan_power_name2)
			return -ENOMEM;
		else
			strcpy(data->wlan_power_name2, power2);
	}
	dev_info(dev, "wlan_power_name2 (%s)\n", data->wlan_power_name2);
