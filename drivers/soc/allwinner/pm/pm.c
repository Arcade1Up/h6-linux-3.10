...
unsigned int standby_stay_cpu;
unsigned int disable_alarm_wakeup;;
#if (defined CONFIG_ARCH_SUN50IW6P1)
unsigned int suspend_hdmi_cec_phyaddr;
EXPORT_SYMBOL_GPL(suspend_hdmi_cec_phyaddr);
#endif
/*
*********************************************************************************************************
*                           aw_pm_valid
...
#endif
	if (disable_alarm_wakeup) {
		super_standby_para_info.event &= ~(CPUS_WAKEUP_ALM0 |
				CPUS_WAKEUP_ALM1);
	}

#if (defined CONFIG_ARCH_SUN50IW6P1)
	super_standby_para_info.hdmi_cec_phyaddr = suspend_hdmi_cec_phyaddr;
#endif
...
static int __init aw_pm_init(void)
{
	...
	pm_np = of_find_node_by_name(NULL, "pm_para");
	if (of_property_read_u32(pm_np, "standby_stay_cpu", &standby_stay_cpu)) {
		pr_err("%s: get standby_stay_cpu failed, default to close\
				vcc-cpu when standby!\n", __func__);
		standby_stay_cpu = 0;
	}

	if (of_property_read_u32(pm_np, "disable_alarm_wakeup", &disable_alarm_wakeup)) {
		pr_err("%s: get disable_alarm_wakeup failed, default to 0!\n", __func__);
		disable_alarm_wakeup = 0;
	}
