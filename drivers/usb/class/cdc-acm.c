...
#define NEOWAY_VENDOR_ID_1 0x3763
...
#define NEOWAY_PRODUCT_N720BZ_8249 0x3C93

bool detected_neoway_product(struct usb_device *dev)
{
	if (NULL == dev)
		return false;
	if ((le16_to_cpu(dev->descriptor.idVendor) == QUALCOMM_VENDOR_ID &&
		(le16_to_cpu(dev->descriptor.idProduct) == NEOWAY_PRODUCT_N720DL ||
		le16_to_cpu(dev->descriptor.idProduct) == NEOWAY_PRODUCT_N720BZ_8241)) ||
		(le16_to_cpu(dev->descriptor.idVendor) == NEOWAY_VENDOR_ID &&
		(le16_to_cpu(dev->descriptor.idProduct) == NEOWAY_PRODUCT_N720BZ_8241 ||
		le16_to_cpu(dev->descriptor.idProduct) == NEOWAY_PRODUCT_N720BZ_8243 ||
		le16_to_cpu(dev->descriptor.idProduct) == NEOWAY_PRODUCT_N720BZ_8247)) ||
		(le16_to_cpu(dev->descriptor.idVendor) == NEOWAY_VENDOR_ID_1 &&
		(le16_to_cpu(dev->descriptor.idProduct) == NEOWAY_PRODUCT_N720BZ_8249))) {
			return true;
	}
	return false;
}
...
static const struct usb_device_id acm_ids[] = {
	...
	{ USB_DEVICE(0x3763, 0x3C93)}, /* N720V5 */
